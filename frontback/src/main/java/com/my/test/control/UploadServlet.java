...
//	}
	 ...
	 private static final int LIMIT_SIZE_BYTES = 1024 * 1024; //첨부파일 최대크기
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            for (FileItem item : items) {
                if (item.isFormField()) { //요청전달데이터
                    System.out.println("파라미터 명 : " + item.getFieldName() //요청전달데이터 이름:
                                    +", 파라미터 값 :  "+item.getString(CHARSET));
                }else { //첨부데이터
                	System.out.println(
                			"파일 파라미터 명 : "+ item.getFieldName()  //요청전달데이터이름 f1
